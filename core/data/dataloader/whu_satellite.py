...
class Whu_satelliteSegmentation(object):

    NUM_CLASS = 2
    def __init__(self, root='/XXXXX/', split='train', mode=None, base_size=256,crop_size=256):
        super(Whu_satelliteSegmentation, self).__init__()
        ...
        _splits_dir = root
        if split == 'train':
            _split_f = os.path.join(_splits_dir, 'train1.txt')
        ...
        self.transform = transforms.Compose([
            transforms.ToTensor(),
            # transforms.Normalize([.387, .395, .403], [.157, .153, .154]),#bcdd
        ])
        ...
        self.test_transform = transforms.Compose([
            transforms.ToTensor(),
            # transforms.Normalize([.368, .400, .388], [.124, .119, .118]),#bcdd
        ])
...
if __name__ == '__main__':
    dataset = Whu_satelliteSegmentation()